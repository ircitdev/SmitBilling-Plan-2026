...
export const RoiCalculatorModal: React.FC<RoiCalculatorModalProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-sm animate-in fade-in duration-200">
      <div 
        className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl max-w-2xl w-full max-h-[90vh] overflow-y-auto shadow-2xl p-6 sm:p-8"
      ...
      >
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-extrabold text-slate-900 dark:text-white">
                Калькулятор окупаемости (ROI) СмИТ Биллинг
              </h2>
              <p className="text-xs text-slate-500 dark:text-slate-400">
                Оцените годовую экономию для вашего интернет-провайдера
              </p>
            </div>
          </div>
        ...
        <div className="py-6 space-y-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div className="p-4 rounded-[22px] bg-slate-50 dark:bg-slate-800/60 border border-slate-200/60 dark:border-slate-700/60">
              <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1.5">
                Сотрудников поддержки (HelpDesk)
              </label>
              <input
                type="number"
                min="1"
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="p-4 rounded-[22px] bg-slate-50 dark:bg-slate-800/60 border border-slate-200/60 dark:border-slate-700/60">
              <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1.5">
                SMS в месяц (уведомления/баланс)
              </label>
              <input
                type="number"
            ...
            <div className="p-4 rounded-[22px] bg-slate-50 dark:bg-slate-800/60 border border-slate-200/60 dark:border-slate-700/60">
              <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1.5">
                Текущие расходы на биллинг (₽/мес)
              </label>
              <input
                type="number"
        ...
        {/* Results Banner */}
        <div className="p-6 rounded-[28px] bg-gradient-to-br from-emerald-500/10 via-emerald-500/5 to-teal-500/10 dark:from-emerald-950/40 dark:via-slate-900 dark:to-teal-950/30 border border-emerald-500/30 space-y-4 shadow-sm">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
            <div>
              <span className="text-[11px] font-extrabold uppercase tracking-wider text-emerald-700 dark:text-emerald-400">
                Итоговая экономия
              </span>
              <div className="text-3xl sm:text-4xl font-extrabold text-emerald-800 dark:text-emerald-300 font-mono">
                {calculations.totalAnnualSavings.toLocaleString('ru-RU')} ₽ / год
              </div>
            </div>

            <div className="sm:text-right">
              <span className="text-xs text-slate-500 block">Срок окупаемости перехода:</span>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 pt-3 border-t border-emerald-500/20 text-xs">
            <div>
              <span className="text-slate-500 block">AI-автоматизация:</span>
              <span className="font-bold text-slate-900 dark:text-white font-mono">
                +{calculations.aiSupportSavingsMonthly.toLocaleString('ru-RU')} ₽/мес
              </span>
            </div>
            <div>
              <span className="text-slate-500 block">Android SMS шлюз:</span>
              <span className="font-bold text-slate-900 dark:text-white font-mono">
                +{calculations.smsSavingsMonthly.toLocaleString('ru-RU')} ₽/мес
              </span>
            </div>
            <div>
              <span className="text-slate-500 block">Снижение оттока:</span>
              <span className="font-bold text-slate-900 dark:text-white font-mono">
                +{calculations.churnSavedMonthly.toLocaleString('ru-RU')} ₽/мес
              </span>
            </div>
            <div>
              <span className="text-slate-500 block">Экономия на ПО:</span>
              <span className="font-bold text-slate-900 dark:text-white font-mono">
        ...
        <div className="mt-6 flex flex-col sm:flex-row items-center justify-between gap-3">
          <span className="text-xs text-slate-400">
            Расчет основан на метриках реального внедрения Build 2286
          </span>
          <button
            onClick={onClose}
