            ...
            >
              ...
              {(item.build || item.planUrl) && (
                <div className="mt-4 pt-3 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between gap-3 flex-wrap">
                  {item.build ? (
                    <span className="text-[11px] font-mono text-slate-400">Build {item.build}</span>
                  ) : (
                    <span />
                  )}
                  {item.planUrl && (
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      title="Открыть страницу плана работ"
